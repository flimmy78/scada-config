...
public class ScadaChannelConfigView extends ViewPart implements
	...
	private String[] comboBaudArray = new String[] { "9600", "19200", "38400",
			"57600", "115200" };
	...
	/*
	 * IEC104下的通讯帧对象模型 【召唤类型编码】-【执行间隔(s)】|【帧名称(可省略)】
	 */
	...
	public void createPartControl(Composite parent) {
		...
		labelProtocal.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, false,
				false, 1, 1));
		...
		button = new Button(groupBasicInfo, SWT.CHECK);
		button.setText("启用");
		button.setSelection(true);

		Label label_2 = new Label(groupBasicInfo, SWT.NONE);
		label_2.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, false, false,
				1, 1));
		label_2.setText("生产厂家：");

		text_cj = new Text(groupBasicInfo, SWT.BORDER);
		text_cj.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false,
				1, 1));

		Label label_3 = new Label(groupBasicInfo, SWT.NONE);
		label_3.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, false, false,
				1, 1));
		label_3.setText("型号：");

		text_xh = new Text(groupBasicInfo, SWT.BORDER);
		text_xh.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false,
				1, 1));

		Label label_4 = new Label(groupBasicInfo, SWT.NONE);
		label_4.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, false, false,
				1, 1));
		label_4.setText("通道地址：");
		...
		GridData gd_tabFolderPortInfor = new GridData(SWT.LEFT, SWT.CENTER,
				false, false, 1, 1);
		gd_tabFolderPortInfor.widthHint = 282;
		tabFolderPortInfor.setLayoutData(gd_tabFolderPortInfor);
		tabFolderPortInfor.setSelectionBackground(Display.getCurrent()
		...
		labelIP.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, false, false,
				1, 1));
		labelIP.setText("IP   ：");

		textIP = new Text(composite_3, SWT.BORDER);
		GridData gd_textIP = new GridData(SWT.FILL, SWT.CENTER, false, false,
				1, 1);
		gd_textIP.widthHint = 144;
		textIP.setLayoutData(gd_textIP);

		Label labelPort = new Label(composite_3, SWT.NONE);
		labelPort.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, false,
				false, 1, 1));
		...
		labelDTUID.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, false,
				false, 1, 1));
		labelDTUID.setText("DTU-ID ：");

		textDTUID = new Text(composite_4, SWT.BORDER);
		textDTUID.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, false,
				false, 1, 1));

		Label labelDtuPort = new Label(composite_4, SWT.NONE);
		labelDtuPort.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, false,
				false, 1, 1));
		labelDtuPort.setText("   端口：");

		textDtuPort = new Text(composite_4, SWT.BORDER);
		textDtuPort.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, false,
				false, 1, 1));

		Label labelHeart = new Label(composite_4, SWT.NONE);
		labelHeart.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, false,
				false, 1, 1));
		labelHeart.setText("心跳信号：");

		textHeart = new Text(composite_4, SWT.BORDER);
		textHeart.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, false,
				false, 1, 1));
		new Label(composite_4, SWT.NONE);
		new Label(composite_4, SWT.NONE);

		Label labelHeartInterval = new Label(composite_4, SWT.NONE);
		labelHeartInterval.setLayoutData(new GridData(SWT.FILL, SWT.CENTER,
				false, false, 1, 1));
		labelHeartInterval.setText("信号间隔：");

		textHeartInteval = new Text(composite_4, SWT.BORDER);
		textHeartInteval.setLayoutData(new GridData(SWT.FILL, SWT.CENTER,
				false, false, 1, 1));
		new Label(composite_4, SWT.NONE);
		new Label(composite_4, SWT.NONE);
		...
		gridViewerColumn.setEditingSupport(new EditingSupport(gridTableViewer) {
			...
			protected void setValue(Object element, Object value) {
				...
				if (deviceAddress.length() == 0 || deviceAddress == null) {
					MessageDialog.openError(gridModbusFrame.getShell(), "错误",
							"设备地址不能为空！");
				...
				.setEditingSupport(new EditingSupport(gridTableViewer) {
					...
					protected void setValue(Object element, Object value) {
						...
						if (functionID.length() == 0 || functionID == null) {
							MessageDialog.openError(gridModbusFrame.getShell(),
									"错误", "功能码不能为空！");
		...
		gridTableViewer_1 = new GridTableViewer(composite_7, SWT.BORDER
				| SWT.H_SCROLL | SWT.V_SCROLL);
		...
		gridViewerColumn_6.setEditingSupport(new EditingSupport(
				gridTableViewer_1) {
			protected boolean canEdit(Object element) {
				return true;
			}
			...
			protected void setValue(Object element, Object value) {
				...
				if (callTypeNum.length() == 0 || callTypeNum == null) {
					MessageDialog.openError(gridIECFrame.getShell(), "错误",
							"召唤类型不能为空！");
		...
		gridViewerColumn_7.setEditingSupport(new EditingSupport(
				gridTableViewer_1) {
			protected boolean canEdit(Object element) {
				return true;
			}
		...
		gridViewerColumn_8.setEditingSupport(new EditingSupport(
				gridTableViewer_1) {
			protected boolean canEdit(Object element) {
				return true;
			}
		...
		btnSave.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				if (acquisitionChannel.getId() == null) {// 添加采集通道
					...
					acquisitionChannel
							.setName(textChannelName.getText().trim());
					acquisitionChannel.setProtocal(getCommPotclType(
							comboProtocal.getText()).getName());
					acquisitionChannel.setIdx(Integer.valueOf(textIdx.getText()
							.trim()));
					acquisitionChannel.setOffline(Integer.valueOf(textOffline
							.getText()));
					acquisitionChannel.setIntvl(Integer.valueOf(textInterval
							.getText()));
					...
					ScadaDeviceTreeView.treeViewer.add("采集通道",
							acquisitionChannel);
					ScadaDeviceTreeView.treeViewer.setExpandedState("采集通道",
							true);
		...
		ViewPropertyChange.getInstance().addPropertyChangeListener("channel",
				this);
	}
	...
	public void propertyChange(PropertyChangeEvent event) {
		if (event.getProperty().equals(
			...
			comboProtocal.setText(acquisitionChannel.getProtocal() == null ? ""
					: getCommPotclType(acquisitionChannel.getProtocal())
							.getValue());
			textIdx.setText(acquisitionChannel.getIdx().toString());
			textOffline.setText(acquisitionChannel.getOffline() + "");
			textInterval.setText(String.valueOf(acquisitionChannel.getIntvl()));
			// textSchedule.setText(acquisitionChannel.getSchedule());
			
			
			button.setSelection(acquisitionChannel.isUsed());
			...
			// Utils.date2CalendarUtil(updateTime);
			...
			if (acquisitionChannel.getProtocal().contains("Modbus")) {
				...
				// 通讯帧操作
				// String frameStr =
				// "1|3-100-10|0|soe4,2|3-100-10|0,3|3-100-10|0|soe3,4|3-100-10|0|soe2,5|3-100-10|0,6|3-100-10|0|soe1"
				// ;//acquisitionChannel.getFrames();
				String[] frameStrArray = frameStr.split(",");
				for (int i = 0; i < frameStrArray.length; i++) {
					framesStr fs = new framesStr();
					// deviceAddress functionID dataAddress dataLength priority
					// frameName
					int eachLength = frameStrArray[i].split("\\|").length;

					fs.setDeviceAddress(frameStrArray[i].split("\\|")[0]);
					fs.setFunctionID(frameStrArray[i].split("\\|")[1]
							.split("-")[0]);
					fs.setDataAddress(frameStrArray[i].split("\\|")[1]
							.split("-")[1]);
					fs.setDataLength(frameStrArray[i].split("\\|")[1]
							.split("-")[2]);
				...
				// String frameStr =
				// "0x65-1200|召唤电能,0x64-10,0x8e-600|召唤定时示功图数据,0x34-10|总召唤,0x64-90,0x64-10|总召唤的"
				// ;//acquisitionChannel.getFrames();
				String[] frameStrArray = frameStr.split(",");
				for (int i = 0; i < frameStrArray.length; i++) {
					...
					if (eachLength == 2) {
						fs.setInterval(frameStrArray[i].split("-")[1]
								.split("\\|")[0]);
						fs.setFrameName(frameStrArray[i].split("-")[1]
								.split("\\|")[1]);
	...
	public void dispose() {
		ViewPropertyChange.getInstance()
				.removePropertyChangeListener("channel");
		super.dispose();
	}
	...
	public void setPortInfro(PropertyChangeEvent event) {
		if (event.getProperty().equals(
			...
			// 20130713,王蓬====
			/**
			 * 物理信息 TCP/IP 通讯方式： 【通讯方式】|【IP】:【端口】 tcp/ip|192.168.1.110:4660 DTU
			 * 通讯方式： 【通讯方式】|【DTU-ID】:【端口】:【心跳信号】:【心跳信号间隔】
			 * dtu|2000:9815:hello:180 串口通讯方式:
			 * 【通讯方式】|【端口】：【波特率】:【数据位(5/6/7/8)】:【校验位(无/奇/偶)】:【停止位(1/1.5/2)】
			 * serial|1:9600:8:无:1
			 */
			// ==========================控制端口信息=================================================
	...
	public String getPortInforStr(int tabIndex) {
		...
		if (tabIndex == 0) { // 当前通讯方式为TCP/IP
			...
			portInforStr = "tcp/ip" + "|" + textIP.getText().trim() + ":"
					+ textPort.getText().trim();

		} else if (tabIndex == 1) { // 当前通讯方式为dtu
			System.out.println("标签栏被选择的索引为： " + "DTU");
			// textDTUID,textDtuPort,textHeart,textHeartInteval
			// dtu|2000:9815:hello:180
			portInforStr = "dtu" + "|" + textDTUID.getText().trim() + ":"
					+ textDtuPort.getText().trim() + ":"
					+ textHeart.getText().trim() + ":"
					+ textHeartInteval.getText().trim();
		} else { // 当前通讯方式为串口Serial
			System.out.println("标签栏被选择的索引为： " + "Serial");
			// textSerialPort,comboBaud,comboDatabit,comboCheckbit,comboStopbit
			// serial|1:9600:8:无:1
			portInforStr = "serial" + "|" + textSerialPort.getText().trim()
					+ ":" + comboBaud.getText().trim() + ":"
					+ comboDatabit.getText().trim() + ":"
					+ comboCheckbit.getText().trim() + ":"
					+ comboStopbit.getText().trim();
		}
		return portInforStr;
	}
	...
	public String getFrameInforStr(int tabIndex) {
		...
		if (tabIndex == 0) { // 通讯规约为modbus
			...
			for (int i = 0; i < gridItems.length; i++) {
				...
				framesInforStr.append(temp1.getDeviceAddress() + "|"
						+ temp1.getFunctionID() + "-" + temp1.getDataAddress()
						+ "-" + temp1.getDataLength() + "|"
						+ temp1.getPriority());
				if (temp1.getFrameName() == null
						|| temp1.getFrameName().length() == 0) {
					// doNothing;
				} else {
			...
			for (int j = 0; j < gridItems.length; j++) {
				...
				// 【召唤类型编码】-【执行间隔(s)】|【帧名称(可省略)】
				framesInforStr.append(temp11.getCallTypeNum() + "-"
						+ temp11.getInterval());
				if (temp11.getFrameName() == null
						|| temp11.getFrameName().length() == 0) {
					// doNothing;
				} else {
