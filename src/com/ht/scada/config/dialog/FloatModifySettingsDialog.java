package com.ht.scada.config.dialog;
...
public class FloatModifySettingsDialog extends Dialog {

	private FloatModifyTypeModel model;
	private Text text;
	 ...
	 * @param parentShell
	 */
	public FloatModifySettingsDialog(Shell parentShell, FloatModifyTypeModel model, int num) {
		super(parentShell);
		this.model = model;
		this.num = num;
	}
	...
	private void initValues() {
		text_base.setText(Float.toString(this.model.getBase()));
		text.setText(Float.toString(this.model.getInterval()));
		if (model.getType() == 0) {
	...
	protected void okPressed() {
		...
		try {
			...
			if (s.startsWith("0x") || s.startsWith("0X")) {
				...
				model.setBase(Float.parseFloat(s));
			}
		...
		if (this.model.getType() != 0) {
			try {
				float interval = Float.parseFloat(this.text.getText().trim());
